...
```text
/mauri-mode add json output to this command. text output stays byte-identical, the json parses, both run against the sample project. show me the evidence.
```
...
An open PR starts collecting blockers immediately. Checks fail, reviewers comment, trunk moves. Hand that churn to the [Babysit playbook](../../skills/mauri-mode/playbooks/babysit.md):

```text
/mauri-mode babysit this pr. get it green.
```
...
```text
/mauri-mode check on pr 123. anything outstanding?
```
