---
name: setup-mstack
description: Configure which models mstack uses per role. Detects your available models and writes an always-applied rule that overrides the skill defaults. Use for /setup-mstack, "configure mstack models", or changing mstack's model choices.
---

# Setup mstack

Write `~/.cursor/rules/mstack-models.mdc`, an always-applied rule that sets mstack's model per role. The skills read it and fall back to their inline defaults when a line is absent, so this is an override layer, not a requirement.
...
The default role-to-model mapping is the rule shape shown in step 5 below. If `~/.cursor/rules/mstack-models.mdc` already exists, read it and treat its values as the current choices. Otherwise start from those defaults.
...
Write `~/.cursor/rules/mstack-models.mdc` with `alwaysApply: true` and one line per role, using the same labels mauri-mode uses. Overwrite the whole file so re-runs stay idempotent. Shape:

```
---
description: mstack per-role model choices (overrides skill defaults)
alwaysApply: true
---
# mstack model configuration. One line per role. Delete a line to fall back to the skill default.
...
Check whether the project has a way to drive the real app for proof (a `verify-*` skill, or an existing harness). If not, offer once: "want a project-local verification skill, so agents can drive the app the way a user does and prove changes work? I can generate one with /create-verification-skill." On yes, invoke `/create-verification-skill` (resolves wherever mstack is installed — workspace, user, or plugin). On no, move on without pushing.
