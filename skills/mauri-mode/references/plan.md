...
Produce a phased implementation plan grounded in the **Principles** section of the `mauri-mode` skill. The plan is the deliverable. Do not implement.
...
Read the **Principles** section of the `mauri-mode` skill end to end, and the leaf `principle-*` skills it indexes. The principles govern every plan decision; cross-link them.
...
- Prefer `subagent_type: "mauri-agent"`. `generalPurpose` is the fallback. Never use the built-in `plan` subagent_type; it ignores this skill.
...
In the overview, name which mauri-mode non-negotiables the implementer must apply, by name:
