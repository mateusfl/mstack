...
4. Verify each component against its baseline via image diff on the matching surface. For browser, web, or Electron components, use `control-lize` first when it is on PATH and follow `../references/control-lize.md` for every state capture. A nonzero diff is a fail. Investigate the pixel delta. `/loop` per component until the diff is zero.
5. Keep the validated changes on one task branch. Run **Opening a PR** only after the user explicitly requests the final delivery. Do not open one PR per component or create a stack by default.
