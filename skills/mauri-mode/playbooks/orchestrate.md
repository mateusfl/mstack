...
**Explicit exception.** This is a standing program workflow that may use many PRs and stacks. Do not route ordinary feature, fix, refactor, or adjustment work here. Use it only when the user explicitly hands over a multi-day program and authorizes its topology. A normal task uses one branch, verification, and a later single PR.
...
- **Coordinator (this chat).** Inspect the actual environment before framing the program. Local or cloud placement is a decision from checkout, tools, credentials, and the live surface, not a fixed role assumption. Frames, authors briefs, drains the inbox, owns the human report, and makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit is bookkeeping only when the authorized program permits it. Agents are spawned, resumed, and drained only through the Task tool. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
...
- **Worker / verifier.** Inspect `pwd`, branch, status, worktrees, remotes, credentials, and the target surface before choosing local or cloud. For browser, web, or Electron proof, use `control-lize` first when it is available and follow `../references/control-lize.md`. Use the project-local `verify-*`, a PATH `control-*` binary, or `control-ui` / `control-cli` from `cursor-team-kit` for other surfaces or as the documented fallback. Never assume a process, auth session, or uncommitted file crosses environments. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
