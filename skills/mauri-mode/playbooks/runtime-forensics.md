...
1. Capture the live signal on the matching surface via the control skill. For a browser, web, or Electron target, use `control-lize` first when it is on PATH and follow `../references/control-lize.md`; use its screenshots and state checks before each diagnostic action. Capture a CPU profile for a spinning process, a heap snapshot for a leak, or a CDP trace for a visual glitch. A real artifact, not a guess.
