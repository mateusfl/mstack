...
- `## Verification`. State how you ran each check and its rigor. Name the harness actually used: the project-local `verify-*` skill, the PATH `control-*` binary it named, `control-ui` / `control-cli` from `cursor-team-kit` only as the fallback, or the targeted tests. State the outcome of each check, not only the command name.
