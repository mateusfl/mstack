...
2. Reconstruct operational state. Read `../references/work-context.md`, then inspect the actual environment, branch and worktree, what already landed (`git log`, `git diff` against the base), the open todos, and the decisions made. The prior trail is authoritative input, but it does not prove that processes, credentials, browser sessions, or uncommitted files exist in this environment. Resist the bias to re-derive the product context while still rechecking the operational context.
